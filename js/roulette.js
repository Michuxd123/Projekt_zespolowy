...
const wheelVisual = document.getElementById('wheel-visual');
    console.log('--- Krok 8: Losuję liczbę... ---');
    const winningNumber = spinWheel();
    const winningColor = ROULETTE_NUMBERS[winningNumber];
    wheelVisual.classList.add('is-spinning-anim');
    wheelVisual.style.borderColor = 'var(--primary-gold)'; 
    resultNumberEl.textContent = ''; 
    
    setTimeout(() => {
        wheelVisual.classList.remove('is-spinning-anim');
        resultNumberEl.textContent = winningNumber;
        if (winningColor === 'red') wheelVisual.style.borderColor = '#d10000';
        else if (winningColor === 'black') wheelVisual.style.borderColor = '#111';
        else wheelVisual.style.borderColor = '#0c6b0c';
        console.log(`--- Krok 9: WYLOSOWANO: ${winningNumber} (${winningColor}) ---`);
        const payoutMultiplier = checkWin(winningNumber);
        if (payoutMultiplier > 0) {
            const winnings = betAmount * payoutMultiplier;
            messageEl.textContent = `Wygrywasz! Trafiono ${winningNumber}. Wygrywasz ${winnings}.`;
